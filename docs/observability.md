...
O que a borda registra, como isso se liga aos logs estruturados que a API e a função serverless já produzem, e o que **não** é responsabilidade dela.

## Índice

- [Log de acesso](#log-de-acesso)
- [Correlação fim a fim](#correlação-fim-a-fim)
- [O endereço de origem é atribuído na borda](#o-endereço-de-origem-é-atribuído-na-borda)
- [Métricas](#métricas)
- [Retenção](#retenção)
- [Como rastrear uma requisição](#como-rastrear-uma-requisição)
- [O que não é responsabilidade da borda](#o-que-não-é-responsabilidade-da-borda)
...
| `sourceIp` | `$context.identity.sourceIp` | Endereço de origem observado pela borda |
...
A ausência da query string é uma escolha: ela é controlada pelo cliente e pode transportar segredo. O `routeKey` já responde "qual rota", que é o que importa na borda.

### Separar falha de borda de falha de aplicação

É o par `status` + `integrationStatus` que faz isso:

| `status` | `integrationStatus` | Leitura |
|---|---|---|
| `401` | `401` | A aplicação recusou. A borda só encaminhou |
| `404` | ausente | **A borda** recusou — caminho ou método não publicado. O backend nem foi tocado |
| `429` | ausente | **A borda** aplicou o limite de frequência |
...
A borda **sobrescreve** `x-request-id` com `$context.requestId` nas **duas** integrações. Como a API e a função serverless preferem o `x-request-id` de entrada, a linha do log de acesso e a linha do log estruturado do backend passam a compartilhar o mesmo identificador.

**Nas duas, não só na do EKS** — é a parte fácil de errar. Sem o mapeamento na integração `AWS_PROXY`, o `resolveCorrelationId(event.headers, context.awsRequestId)` da função cairia no `awsRequestId`, que é o identificador da **Lambda**, diferente do `$context.requestId` do log de acesso. A correlação quebraria exatamente na rota que esta borda existe para publicar.

> **Um defeito encontrado na verificação, e corrigido.** A premissa de que "os backends já leem `x-request-id`" estava certa mas incompleta: os dois validavam o valor de entrada com um alfabeto que **não aceitava `=`**, e o `$context.requestId` do API Gateway é base64 e termina exatamente nesse caractere. Os dois recusavam o identificador da borda e geravam um UUID próprio. O charset foi ampliado nos dois repositórios, sem afrouxar a validação de conteúdo — quem recusa JWT e documento formatado é o sanitizador, não o alfabeto.
...
## O endereço de origem é atribuído na borda

**A aplicação não enxerga o IP do cliente, e isso é decisão fechada — não pendência.**

Quatro fatos da plataforma, em ordem:

1. **HTTP APIs convertem a família `X-Forwarded-*` no cabeçalho padrão `Forwarded`** (RFC 7239). O `X-Forwarded-For` que o backend eventualmente veria é o salto da própria borda, não o cliente.
...
Observado no ambiente provisionado: a aplicação registra `client.address = "::ffff:10.0.10.68"` — a ENI do NLB, com `preserve_client_ip = false`. O endereço do cliente está na linha da borda, ligado ao resto pelo mesmo `x-request-id`.
...
**A granularidade é a da rota do Gateway, não a dos endpoints da aplicação.** Se um único endpoint servido pelo proxy passar a falhar, as métricas da borda apontam a rota de proxy — a identificação do endpoint vem do log estruturado da aplicação, pelo `http.route`. A borda não promete métricas por endpoint, e não deve ser lida como se prometesse.
...
Os dois lados registram o mesmo valor: `requestId` na borda, `request.id` na aplicação. A partir daí, o log da aplicação dá o que a borda não tem — `http.route` real, duração interna, contexto de negócio — e a borda dá o que a aplicação não tem: o endereço de origem e a latência total incluindo a rede.

Para a função serverless, o mesmo identificador aparece no log de invocação dela.

## O que não é responsabilidade da borda
...
| Monitor sintético contínuo | **Não existe.** A borda torna possível verificar de fora, mas a verificação é manual e pontual — ver [ci-cd.md](ci-cd.md#verificação-externa-após-o-provisionamento) |
