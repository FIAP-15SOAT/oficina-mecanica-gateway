...
Um API Gateway é o lugar convencional para colocar autenticação: o API Gateway rejeita o tráfego não autenticado antes de ele custar recurso de backend, e a política fica declarada em um só lugar. A pergunta "por que não há autorizador aqui?" vai ser feita, e merece uma resposta que não seja "ficou para depois".
...
O Gateway **não autentica e não autoriza**. Ele encaminha o cabeçalho de autorização intacto, não o inspeciona, não o valida e não o registra em log. A única responsabilidade que assume no fluxo de autenticação é o **throttling** das duas rotas de login — que é exatamente o que a `docs/security.md` da função serverless já delega ao API Gateway.
...
### O que o API Gateway faz no fluxo de autenticação
...
- **Lambda authorizer.** Tecnicamente possível. Exigiria colocar o `JWT_SECRET` (HS256) **e** a chave pública RS256 no API Gateway — piora a custódia de segredos sem decidir nada de fato, porque a decisão real depende do banco. Confirmado com o time como fora de escopo.
...
- Nenhum segredo de assinatura é copiado para o API Gateway.
